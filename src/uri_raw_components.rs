...
impl<'a> UriRawComponents<'a> {
    ...
    pub fn from_str(uri: &'a str) -> Result<UriRawComponents<'a>, ParseError> {
        ...
        let components = match parse_uri_components(uri) {
            Some(x) => x,
            None => {
        ...
        let scheme = if let Some(s) = components.scheme {
            ...
            if is_valid_scheme(s) {
                ...
                return Err(ParseError::new(
                    "Invalid URI scheme",
                    Some(components.scheme_start..components.scheme_end),
                ));
            }
        } else {
            None
        ...
        let authority = components.authority;
...
impl<'a> UriRawComponents<'a> {
    ...
    pub unsafe fn from_components_unchecked(
        ...
        if let Some(authority) = authority {
            match parse_authority_components(authority) {
                Some(parts) => {
                    userinfo = parts.userinfo;
                    host = parts.host;
                    port = parts.port.and_then(|p| u16::from_str(p).ok());
                }
                None => {
                    userinfo = None;
                    host = None;
